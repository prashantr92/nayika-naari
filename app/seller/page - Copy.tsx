...
export default function SellerDashboard() {
 ...
 const [dashStats, setDashStats] = useState({
    ...
    yesterday: { active: 0, new: 0, buyers: 0, orders: 0, carts: 0 } // 🌟 NAYA: 'week' ki jagah 'yesterday'
  });
  ...
  const fetchDashboardStats = async () => {
    try {
      ...
      // 🌟 NAYA: Today aur Yesterday ki exact timing
      ...
      const [{ data: oData }, { data: uData }, { data: cData }] = await Promise.all([
        // 🌟 FIX: '.gte()' hata diya taaki poore DB se 'first order' ka pata chal sake
        supabase.from('orders').select('id, userid, createdAt, created_by'),
        supabase.from('users').select('id, created_at, meta'),
        supabase.from('cart_items').select('user_id, updated_at, product_id').gte('updated_at', startOfYesterdayISO)
      ]);
      ...
      let tActive = 0, tNew = 0, yActive = 0, yNew = 0;
      let tOrders = 0, yOrders = 0, tCarts = new Set(), yCarts = new Set();

      // 🌟 TRUE NEW BUYERS LOGIC (First Order Check)
      const userFirstOrderMap = new Map();
      validOrders.forEach(o => {
         const time = new Date(o.createdAt).getTime();
         if (!userFirstOrderMap.has(o.userid) || time < userFirstOrderMap.get(o.userid)) {
             userFirstOrderMap.set(o.userid, time);
         }
      });

      let tNewBuyers = 0, yNewBuyers = 0;
      userFirstOrderMap.forEach((firstOrderTime) => {
         if (firstOrderTime >= startOfTodayMs) tNewBuyers++;
         else if (firstOrderTime >= startOfYesterdayMs && firstOrderTime < startOfTodayMs) yNewBuyers++;
      });

      // Orders & Carts logic
      validOrders.forEach(o => {
        const time = new Date(o.createdAt).getTime();
        if (time >= startOfTodayMs) tOrders++;
        else if (time >= startOfYesterdayMs && time < startOfTodayMs) yOrders++;
      });

      validCarts.forEach(c => {
        const time = new Date(c.updated_at).getTime();
        if (time >= startOfTodayMs) tCarts.add(c.user_id);
        else if (time >= startOfYesterdayMs && time < startOfTodayMs) yCarts.add(c.user_id);
      });
      
      const sellerUserIds = new Set([...validOrders.map(o=>o.userid), ...validCarts.map(c=>c.user_id)]);
      ...
      (uData || []).forEach(u => {
        ...
        if (u.created_at) {
          ...
          else if (ct >= startOfYesterdayMs && ct < startOfTodayMs) yNew++;
        }
        let metaObj = safeParseJSON(u.meta, {});
        if (metaObj.lastSeen) {
          const ls = new Date(metaObj.lastSeen).getTime();
          if (ls >= startOfTodayMs) tActive++;
          else if (ls >= startOfYesterdayMs && ls < startOfTodayMs) yActive++;
        }
      });

      setDashStats({ 
        today: { active: tActive, new: tNew, buyers: tNewBuyers, orders: tOrders, carts: tCarts.size }, 
        yesterday: { active: yActive, new: yNew, buyers: yNewBuyers, orders: yOrders, carts: yCarts.size } 
      });
...
const handleProductToggle = async (product: any) => {
  ...
  const fetchUsersData = async () => {
    ...
    // 🌟 FIX: Leads data sirf Admin ya banane wale Seller ko dikhega
    let userBaseQuery = supabase.from('user_base').select('*').order('created_at', { ascending: false });
    if (!isAdmin) {
       userBaseQuery = userBaseQuery.eq('created_by', currentUser?.id);
    }
    ...
    if (users) {
      const enrichedUsers = users.map(u => {
        ...
        // 🌟 FIX: Pre-parse logs and filter notes added by current user
        const parsedLogs = Array.isArray(u.logs) ? u.logs : safeParseJSON(u.logs, []);
        const parsedNotes = parsedLogs.filter((l: any) => l.type === 'note' || l.text).filter((n: any) => isAdmin || n.addedBy === currentUser?.id);

        return { ...u, orderCount: uOrders.length, lastOrder, lastCart, cartUnique: new Set(uCart.map(c => c.product_id)).size, cartPcs: uCart.reduce((acc, c) => acc + c.qty, 0), cartBoxes: totalBoxes, lastSeen: safeParseJSON(u.meta, {}).lastSeen || null, uCart, logs: parsedLogs, notes: parsedNotes };
      }).filter(Boolean);
      ...
      const baseList = (userBase || []).map(b => {
          ...
          const allNotesRaw = safeParseJSON(b.followupnotes, []);
          const myNotes = allNotesRaw.filter((n: any) => isAdmin || n.addedBy === currentUser?.id);
          return { ...b, appData: matchedAppUser, notes: myNotes, allNotesRaw: allNotesRaw };
      });
 ...
 // 🌟 NAYA: Image Compression Helper (Bina quality lose kiye size chota karega)
  ...
  // 🌟 TUMHARA MAIN UPLOAD FUNCTION (Fixed Silent Fails & Meta Merge)
  const handleUploadProduct = async () => {
    // 🌟 FIX: !uploadForm.mrp yahan se hata diya taaki ye non-mandatory ho jaye
    if (!uploadForm.name || !uploadForm.subcategory || !uploadForm.cost || (uploadImages.length === 0 && existingImages.length === 0)) return alert("Fill required fields and add 1 image.");
    setIsUploading(true);
    try {
      ...
      // 🌟 FIX 1: Pichla META data merge kiya, taaki kuch accidentally delete na ho
     ...
     // 🌟 NAYA: Cost, MRP aur Discount ka calculation
      const parsedCost = parseFloat(uploadForm.cost) || 0;
      ...
      // 🌟 FIX 2: Condition laga di. Edit mein DB ka seller nahi chhedenge, sirf naye upload mein name bhejenge
      if (isEditMode && editingProductId) {
        const { error } = await supabase.from('products').update(productData).eq('id', editingProductId);
        if (error) throw error;
      } else {
        // Naya product upload hote waqt seller ka naam (number nahi) jayega
        productData.seller = currentUser.name; 
        const { error } = await supabase.from('products').insert([productData]);
        if (error) throw error;
      }
      
      showToast(isEditMode ? "Updated!" : "Uploaded!"); 
      resetUploadForm(); 
      fetchMyProducts();
      if (isEditMode) setView('products'); 
      else window.scrollTo(0,0);
    } catch (e: any) { 
      // Ab error aayega toh exact screen par likha aayega kya fail hua
 ...
 const saveVariantStatus = async () => { 
    ...
    try { 
      const meta = safeParseJSON(editingProduct.meta, {}); 
      meta.attributes.available_sizes = editingSizeConfig; 
      
      // 🌟 FIX: Error check add kiya
      const { error } = await supabase.from('products').update({ meta: JSON.stringify(meta) }).eq('id', editingProduct.id); 
      if (error) throw error;
  ...
  const processedAppUsers = useMemo(() => {
      return [...filteredUsers].sort((a, b) => {
          if (appUserSort === 'followup') {
              const getFollowUpDiff = (item: any) => {
                  const notes = item.notes || []; // 🌟 FIX: Direct access, zero processing overhead
                  if (notes.length === 0) return Number.MAX_SAFE_INTEGER;
                  const lastDate = notes[notes.length - 1].nextDate;
                  if (!lastDate) return Number.MAX_SAFE_INTEGER;
...
// 🌟 FINAL PERFORMANCE FIX: UI DOM MEMOIZATION (Lock the Cards)
  ...
  const RenderedAppUsersList = useMemo(() => {
    ...
    return processedAppUsers.map((u, index) => {
      const userNotes = u.notes || []; // 🌟 FIX: Instant UI render
        ...
        <div key={u.id || index} className="bg-white rounded-2xl p-4 border border-gray-200 shadow-sm transition-transform">
          <div className="flex justify-between items-start mb-4">
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              ...
              {/* 🌟 FIX: Phone and WA hide for seller */}
              {isAdmin && (
                <>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden mt-6">
              ...
              <div className="p-0">
                <table className="w-full text-left text-sm">
                    <thead><tr className="bg-gray-50/30 text-[9px] text-gray-400 uppercase tracking-widest"><th className="p-3 font-bold">Metric</th><th className="p-3 font-bold text-center border-l border-gray-100 bg-blue-50/30 text-blue-600">Today</th><th className="p-3 font-bold text-center border-l border-gray-100">Yesterday</th></tr></thead>
                    <tbody className="font-semibold text-gray-700">
                      <tr className="border-t border-gray-100 hover:bg-gray-50 transition-colors"><td className="p-3 text-[11px] flex items-center gap-2"><Activity size={14} className="text-green-500"/> Active Users</td><td className="p-3 text-center border-l border-gray-100 text-gray-900 font-black bg-blue-50/30">{dashStats.today.active}</td><td className="p-3 text-center border-l border-gray-100">{dashStats.yesterday.active}</td></tr>
                      <tr className="border-t border-gray-100 hover:bg-gray-50 transition-colors"><td className="p-3 text-[11px] flex items-center gap-2"><UserPlus size={14} className="text-purple-500"/> New Users</td><td className="p-3 text-center border-l border-gray-100 text-gray-900 font-black bg-blue-50/30">{dashStats.today.new}</td><td className="p-3 text-center border-l border-gray-100">{dashStats.yesterday.new}</td></tr>
                      <tr className="border-t border-gray-100 hover:bg-gray-50 transition-colors"><td className="p-3 text-[11px] flex items-center gap-2"><ShoppingBag size={14} className="text-orange-500"/> New Buyers</td><td className="p-3 text-center border-l border-gray-100 text-gray-900 font-black bg-blue-50/30">{dashStats.today.buyers}</td><td className="p-3 text-center border-l border-gray-100">{dashStats.yesterday.buyers}</td></tr>
                      <tr className="border-t border-gray-100 hover:bg-gray-50 transition-colors"><td className="p-3 text-[11px] flex items-center gap-2"><ClipboardList size={14} className="text-blue-500"/> Orders Placed</td><td className="p-3 text-center border-l border-gray-100 text-gray-900 font-black bg-blue-50/30">{dashStats.today.orders}</td><td className="p-3 text-center border-l border-gray-100">{dashStats.yesterday.orders}</td></tr>
                      <tr className="border-t border-gray-100 hover:bg-gray-50 transition-colors"><td className="p-3 text-[11px] flex items-center gap-2"><ShoppingCart size={14} className="text-pink-500"/> Truck Updated</td><td className="p-3 text-center border-l border-gray-100 text-gray-900 font-black bg-blue-50/30">{dashStats.today.carts}</td><td className="p-3 text-center border-l border-gray-100">{dashStats.yesterday.carts}</td></tr>
                    </tbody>
                 </table>
              </div>
            </div>
            ...
            <div className="bg-white border-b border-gray-100 flex flex-col sticky top-0 z-30 shadow-sm">
              ...
              <div className="px-3 pb-3 flex flex-col gap-3">
                <div className="flex items-center gap-2 overflow-x-auto scrollbar-hide w-full pb-1">
                   ...
                   {/* 🌟 FIX: Only Admin can add lead/user */}
                   {isAdmin && (
                    ...
                    <div key={o.id} onClick={() => openOrderDetails(o)} className="bg-white rounded-2xl p-4 border border-gray-200 shadow-sm cursor-pointer active:scale-[0.98] transition-transform">
                      ...
                      <div className="mt-3 flex items-center justify-between w-full">
                        <div className="flex items-center gap-2">
                          ...
                          {/* 🌟 NAYA: Status ke theek bagal mein Seller Name */}
                          {safeParseJSON(o.meta, {})?.sellerName && (
                            <span className="text-[10px] font-bold text-gray-700 bg-gray-100 px-2 py-1 rounded-md border border-gray-200 flex items-center gap-1 shrink-0">
                              <User size={10} className="text-gray-500" /> {safeParseJSON(o.meta, {}).sellerName}
                            </span>
                          )}
        ...
        {view === 'order_detail' && selectedOrder && (
          <div className="animate-in slide-in-from-right duration-300 flex flex-col min-h-full bg-[#F8F9FA] pb-24">
            ...
            <div className="bg-white p-4 border-b border-gray-200 shadow-sm sticky top-0 z-30 print-hidden">
              <div className="flex justify-between items-start mb-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {/* 🌟 FIX: Edit Status only for admin */}
                  {isAdmin ? (
                          ...
                          return (
                            <div key={item.id} className="flex justify-between items-center text-sm font-bold bg-white border border-gray-100 p-2 rounded-lg">
                              ...
                              <div className="w-20 flex flex-col items-center">
                                {/* 🌟 FIX: Admin edit, Seller view-only */}
                                {isAdmin ? (
                                    ...
                                    <span className="w-14 text-center bg-gray-50 border border-gray-200 text-gray-700 rounded-md py-1 font-black inline-block">{currentQ}</span>
                                )}
            ...
            {isAdmin && (
              ...
              )} {/* 🌟 FIX: Ye line missing thi (isAdmin ka closing bracket) */}
            </div>
          )}
                ...
                onClick={async () => {
                   ...
                   try {
                     ...
                     if (selectedBuyerForNote.isAppUser) {
                         // 🌟 FIX: addedBy property laga di
                         const newLog = { type: 'note', date: getLocalTimestamp(), text: noteForm.text, nextDate: noteForm.nextDate, tag: noteForm.tag, addedBy: currentUser.id };
                         ...
                         const updatedNotes = [...(selectedBuyerForNote.notes || []), newLog];
                         
                         const { error } = await supabase.from('users').update({ logs: updatedLogs }).eq('id', selectedBuyerForNote.id);
                         if(error) throw error;
                         
                         setUsersList(prev => prev.map(u => u.id === selectedBuyerForNote.id ? { ...u, logs: updatedLogs, notes: updatedNotes, latestCrmTag: noteForm.tag } : u));
                     } 
                     else {
                         // 🌟 FIX: addedBy property laga di
                         const newNote = { date: getLocalTimestamp(), text: noteForm.text, nextDate: noteForm.nextDate, tag: noteForm.tag, addedBy: currentUser.id };
                         
                         // allNotesRaw ka use taaki kisi aur (admin) ke notes remove na hon
                         const updatedNotes = [...(selectedBuyerForNote.allNotesRaw || selectedBuyerForNote.notes || []), newNote];
                         ...
                         setBuyersList(prev => prev.map(b => String(b.phone) === String(selectedBuyerForNote.phone) ? { ...b, followupnotes: updatedNotes, notes: [...(b.notes || []), newNote], allNotesRaw: updatedNotes, latestCrmTag: noteForm.tag } : b));
                     }
